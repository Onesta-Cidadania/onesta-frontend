 ...
 * { customerName, customerCode, customerEmail, previousStatus, newStatus, userEmail, userRole }
 * 
 * Body esperado (lote):
 * { changes: [{ customerCode, customerEmail, previousStatus, newStatus }], userEmail, userRole }
 */
router.post('/api/emails/status-change', async (req, res, next) => {
  try {
    ...
    const subject = isBatch
      ? `Alteração de Status - ${changes.length} ${changes.length === 1 ? 'cliente' : 'clientes'}`
      : `Alteração de Status - ${customerName} - ${customerCode || 'N/A'}`;
    ...
    console.log(`📧 Enviando email de alteração de status: ${isBatch ? `${changes.length} clientes` : customerName} por ${userEmail}`);
